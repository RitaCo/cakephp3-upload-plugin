...
namespace RitaUpload\Model\Behavior;
...
use RitaUpload\Lib\Error\Exception;
...
/**
 * Store an instance of the table class
 *
...
/**
 * Merge the default config into the upload field
 *
 * @param Table $table
 * @param array $config
 */
	public function __construct(Table $table, array $config) {
		foreach ($config['uploadFields'] as $field => $options) {
			$this->config('uploadFields.' . $field, array_merge($options, $this->_defaultConfig));
		}
 ...
 */
	protected function _setupField(Model $model, $field, $options) {
		trigger_error(__METHOD__ . ' is deprecated', E_DEPRECATED);
 ...
 */
	public function uploadSettings(Model $model, $field, $one, $two = null) {
		if (empty($this->config[$model->alias][$field])) {
 ...
 */
	public function beforeSave(Event $event, Entity $entity, ArrayObject $options) {
		...
		$isUpdating = !empty($entity->get($this->Table->primaryKey()));

		foreach ($this->config('uploadFields') as $field => $options) {
			if (!isset($entity->{$field}) || !is_array($entity->{$field})) {
				// it may have previously been set by a prior save using this same instance
				unset($this->runtime[$this->Table->alias()][$field]);
				continue;
			}

			$this->runtime[$this->Table->alias()][$field] = $entity->{$field};

			$removing = !empty($entity->{$field}->remove);
			if ($this->_shouldUpdate($entity, $field, $removing)) {
				// We're updating the file, remove old versions
				if ($isUpdating) {
					$data = $this->Table->find()
						->where(['id' => $entity->get($this->Table->primaryKey())])
						->first();

					$this->_prepareFilesForDeletion($entity, $field, $data, $options);
				}

				if ($removing) {
					$entity = $this->Table->patchEntity($entity, array(
						$field => null,
			...
			} elseif (!isset($entity->{$field}->name) || !strlen($entity->{$field}->name)) {
				// if field is empty, don't delete/nullify existing file
				unset($entity->{$field}->name);
				continue;
			}

			$this->runtime[$this->Table->alias()][$field]['name'] = $this->_retrieveName(
				$entity, $field, $this->runtime[$this->Table->alias()][$field]['name'], $entity, array(
					'saveType' => $isUpdating ? 'update' : 'create',
			));

			$entity = $this->Table->patchEntity($entity, array(
				$field => $this->runtime[$this->Table->alias()][$field]['name'],
				$options['fields']['type'] => $this->runtime[$this->Table->alias()][$field]['type'],
				$options['fields']['size'] => $this->runtime[$this->Table->alias()][$field]['size']
			));
 ...
 */
	public function beforeValidate(Event $event, Entity $entity, ArrayObject $options) {
		foreach ($this->config('uploadFields') as $field => $options) {
			if (!empty($entity->{$field}) && $this->_isUrl($entity->{$field})) {
				$uri = $entity->{$field};
				if (!$this->_grab($entity, $field, $uri)) {
					$entity->invalidate($field, __d('upload', 'File was not downloaded.', true));
					return false;
				}
			}
		}
 ...
 */
	public function afterSave(Event $event, Entity $entity, ArrayObject $options) {
		$temp = array($this->Table->alias() => array());

		foreach ($this->config('uploadFields') as $field => $options) {
			if ($this->_shouldSkip($entity, $field)) {
				continue;
			}

			$tempPath = $this->_getPath($entity, $field);

			$path = $this->config('uploadFields.' . $field . '.path');
			$thumbnailPath = $this->config('uploadFields.' . $field . '.thumbnailPath');
			...
			$tmp = $this->runtime[$this->Table->alias()][$field]['tmp_name'];
			$filePath = $path . $entity->{$field};
			if (!$this->handleUploadedFile($entity, $field, $tmp, $filePath)) {
				CakeLog::error(sprintf('Model %s, Field %s: Unable to move the uploaded file to %s', $this->Table->alias(), $field, $filePath));
				$entity->invalidate($field, sprintf('Unable to move the uploaded file to %s', $filePath));
			...
			if ($entity->has($options['fields']['dir'])) {
				if (!($created && $options['pathMethod'] == '_getPathFlat') && $options['saveDir']) {
					$db = ConnectionManager::get('default');
					$temp[$this->Table->alias()][$options['fields']['dir']] = $db->value($tempPath, 'string');
				}
			}
		}

		$this->_updateRecord($entity, $temp);
		return $this->_unlinkFiles($entity);
	}
 ...
 * @return bool
 **/
	public function handleUploadedFile(Entity $entity, $field, $filename, $destination) {
		$callback = Hash::get($this->config('uploadFields.' . $field), 'handleUploadedFileCallback');
		if (is_callable(array($entity, $callback), true)) {
			return $entity->{$callback}($field, $filename, $destination);
		}
 ...
 * @return bool
 **/
	public function deleteFolder(Entity $entity, $path) {
		if (!isset($this->__foldersToRemove[$this->Table->alias()])) {
			return false;
		}

		$folders = $this->__foldersToRemove[$this->Table->alias()];
 ...
 */
	public function afterDelete(Event $event, Entity $entity, ArrayObject $options) {
		$result = array();
		if (!empty($this->__filesToRemove[$this->Table->alias()])) {
			foreach ($this->__filesToRemove[$this->Table->alias()] as $i => $file) {
				$result[] = $this->unlink($file);
				unset($this->__filesToRemove[$this->Table->alias()][$i]);
			}
		}

		foreach ($this->config() as $options) {
			if ($options['deleteFolderOnDelete'] == true) {
				$this->deleteFolder($entity, $options['path']);
				return true;
			}
		}
		return $result;
 ...
 * @return bool Success
 */
	public function isUnderPhpSizeLimit(Entity $entity, $check) {
 ...
 * @return bool Success
 */
	public function isUnderFormSizeLimit(Entity $entity, $check) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isCompletedUpload(Entity $entity, $check) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isFileUpload(Entity $entity, $check) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isFileUploadOrHasExistingValue(Entity $entity, $check) {
 ...
 * @return bool Success
 */
	public function tempDirExists(Entity $entity, $check, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isSuccessfulWrite(Entity $entity, $check, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function noPhpExtensionErrors(Entity $entity, $check, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isValidMimeType(Entity $entity, $check, $mimetypes = array(), $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isWritable(Entity $entity, $check, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isValidDir(Entity $entity, $check, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isBelowMaxSize(Entity $entity, $check, $size = null, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isAboveMinSize(Entity $entity, $check, $size = null, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isValidExtension(Entity $entity, $check, $extensions = array(), $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isAboveMinHeight(Entity $entity, $check, $height = null, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isBelowMaxHeight(Entity $entity, $check, $height = null, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isAboveMinWidth(Entity $entity, $check, $width = null, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 * @return bool Success
 */
	public function isBelowMaxWidth(Entity $entity, $check, $width = null, $requireUpload = true) {
		$field = $this->_getField($check);
 ...
 */
	protected function _getRootDir($rootDir = null) {
		if ($rootDir === null) {
			$rootDir = $this->_defaultConfig['rootDir'];
		}
 ...
 * @return string
 */
	protected function _shouldUpdate(Entity $entity, $field, $removing) {
		if ($removing) {
			return true;
		}

		$deleteOnUpdate = $this->config("uploadFields.$field.deleteOnUpdate");
		$v = $this->config("uploadFields.$field.name");
		$isset = empty($v);
		return $deleteOnUpdate && $isset && strlen($this->config("uploadFields.$field.name"));
	}
 ...
 * @return string
 */
	protected function _retrieveName(Entity $entity, $field, $currentName, $data, $options = array()) {
		$options = array_merge(array(
			'rootDir' => $this->config("uploadFields.$field.rootDir"),
			'saveType' => 'create',
			'geometry' => null,
			'size' => null,
			'thumbnailType' => null,
		...
		$name = $currentName;
		$callback = Hash::get($this->config("uploadFields.$field"), 'nameCallback');

		if (!empty($callback)) {
			$newName = null;

			if (is_callable(array($entity, $callback), true)) {
				$newName = $entity->{$callback}($field, $currentName, $data, $options);
			}
 ...
 * @return mixed
 */
	protected function _unlinkFiles(Entity $entity) {
		if (empty($this->__filesToRemove[$this->Table->alias()])) {
			return true;
		}
 ...
 * @return void
 */
	protected function _updateRecord(Entity $entity, $data) {
		if (!empty($data[$this->Table->alias()])) {
			$this->Table->updateAll($data[$this->Table->alias()], array(
				$this->Table->alias() . '.' . $this->Table->primaryKey() => $entity->get($this->Table->primaryKey())
			));
		}
	}
 ...
 * @return bool
 */
	protected function _shouldSkip(Entity $entity, $field) {
		if (!in_array($field, array_keys($this->Table->alias()))) {
			return true;
		}

		if (empty($this->runtime[$this->Table->alias()][$field])) {
			return true;
		}
		...
		if (empty($entity->data[$this->Table->alias()][$field])) {
			return true;
		}
 ...
 * @return bool
 */
	protected function _resizeImagick(Entity $entity, $field, $path, $size, $geometry, $thumbnailPath) {
		$srcFile = $path . $entity->{$field};
		$pathInfo = $this->_pathinfo($srcFile);
		$thumbnailType = $imageFormat = $this->config("uploadFields.$field.thumbnailType");

		$isMedia = $this->_isMedia($this->runtime[$this->Table->alias][$field]['type']);
		$image = new imagick();
		...
		if ($isMedia) {
			$thumbnailType = $imageFormat = $this->config("uploadFields.$field.mediaThumbnailType");
		}
		...
		$fileName = str_replace(
			...
			array($size, $geometry, $pathInfo['filename'], $entity->get($this->Table->primaryKey())),
			$this->config("uploadFields.$field.thumbnailName")
		);

		$destFile = "{$thumbnailPath}{$fileName}.{$thumbnailType}";

		$image->setImageCompressionQuality($this->config("uploadFields.$field.thumbnailQuality"));
 ...
 * @return bool
 */
	protected function _resizePhp(Entity $entity, $field, $path, $size, $geometry, $thumbnailPath) {
		$srcFile = $path . $entity->{$field};
		$pathInfo = $this->_pathinfo($srcFile);
		$thumbnailType = $this->config("uploadFields.$field.thumbnailType");
		...
		$adjustedQuality = $this->config("uploadFields.$field.thumbnailQuality");
		switch (strtolower($thumbnailType)) {
			...
			case 'png':
				...
				$adjustedQuality = intval((100 - $this->config("uploadFields.$field.thumbnailQuality")) / 100 * 9);
				break;
			default:
				return false;
		}
 ...
 * @return string
 */
	protected function _getPath(Entity $entity, $field) {
		$path = $this->config("uploadFields.$field.path");
		$pathMethod = $this->config("uploadFields.$field.pathMethod");

		if (method_exists($this, $pathMethod)) {
			return $this->$pathMethod($entity, $field, $path);
		}
 ...
 * @return string
 */
	protected function _getPathFlat(Entity $entity, $field, $path) {
		$destDir = $path;
		$this->_mkPath($entity, $field, $destDir);
		return '';
	}
 ...
 * @return string
 */
	protected function _getPathPrimaryKey(Entity $entity, $field, $path) {
		$destDir = $path . $entity->get($this->Table->primaryKey()) . DIRECTORY_SEPARATOR;
		$this->_mkPath($entity, $field, $destDir);
		return $entity->get($this->Table->primaryKey());
	}
 ...
 * @return string
 */
	protected function _getPathRandom(Entity $entity, $field, $path) {
		$endPath = null;
		$decrement = 0;
 ...
 * @return string
 */
	protected function _getPathRandomCombined(Entity $entity, $field, $path) {
		$endPath = $entity->get($this->Table->primaryKey()) . DIRECTORY_SEPARATOR;
		$decrement = 0;
		$string = crc32($field . microtime() . $entity->get($this->Table->primaryKey()));
 ...
 * @return bool
 */
	protected function _grab(Entity $entity, $field, $uri) {
		$socket = new Client(array(
		...
		if (isset($entity->file_name_override)) {
			$fileName = $entity->file_name_override . '.' . pathinfo($socket->request['uri']['path'], PATHINFO_EXTENSION);
		}
 ...
 * @return bool
 */
	protected function _mkPath(Entity $entity, $field, $destDir) {
		if (!file_exists($destDir)) {
			mkdir($destDir, $this->config("uploadFields.$field.mode"), true);
			chmod($destDir, $this->config("uploadFields.$field.mode"));
		}
		return true;
	}
 ...
 * @return string
 **/
	protected function _path(Entity $entity, $field, $options = array()) {
		$defaults = array(
			'isThumbnail' => true,
			'path' => '{ROOT}webroot{DS}files{DS}{model}{DS}{field}{DS}',
			'rootDir' => $this->_defaultConfig['rootDir'],
		...
		$replacements = array(
			...
			'{primaryKey}' => $entity->get($this->Table->primaryKey()),
 ...
 * @return string
 **/
	protected function _pathThumbnail(Entity $entity, $field, $params = array()) {
		return str_replace(
			array('{size}', '{geometry}'),
 ...
 * @throws Exception
 */
	protected function _createThumbnails(Entity $entity, $field, $path, $thumbnailPath) {
		$isImage = $this->_isImage($this->runtime[$this->Table->alias()][$field]['type']);
		$isMedia = $this->_isMedia($this->runtime[$this->Table->alias()][$field]['type']);
		$createThumbnails = $this->settings[$this->Table->alias()][$field]['thumbnails'];
		$hasThumbnails = !empty($this->settings[$this->Table->alias()][$field]['thumbnailSizes']);

		if (($isImage || $isMedia) && $createThumbnails && $hasThumbnails) {
			$method = $this->config("uploadFields.$field.thumbnailMethod");

			foreach ($this->config("uploadFields.$field.thumbnailSizes") as $size => $geometry) {
				$thumbnailPathSized = $this->_pathThumbnail($entity, $field, compact(
					'geometry', 'size', 'thumbnailPath'
				));
				$this->_mkPath($entity, $field, $thumbnailPathSized);

				$valid = false;
				if (method_exists($entity, $method)) {
					$valid = $entity->$method($entity, $field, $path, $size, $geometry, $thumbnailPathSized);
				} elseif (method_exists($this, $method)) {
					$valid = $this->$method($entity, $field, $path, $size, $geometry, $thumbnailPathSized);
				} else {
					CakeLog::error(sprintf('Model %s, Field %s: Invalid thumbnailMethod %s', $this->Table->alias(), $field, $method));
					$db = ConnectionManager::get('default');
					$db->rollback();
				...
				if (!$valid) {
					$entity->invalidate($field, 'resizeFail');
				}
			}
		}
	}
 ...
 * @return bool
 **/
	protected function _prepareFilesForDeletion(Entity $entity, $field, $data, $options = array()) {
		if ($options['keepFilesOnDelete'] === true) {
			return array();
		}

		if (!strlen($data[$this->Table->alias()][$field])) {
			return $this->__filesToRemove;
		}

		if (!empty($options['fields']['dir']) && isset($data[$this->Table->alias()][$options['fields']['dir']])) {
			$dir = $data[$this->Table->alias()][$options['fields']['dir']];
		} else {
			if (in_array($options['pathMethod'], array('_getPathFlat', '_getPathPrimaryKey'))) {
				//$entity->get($this->Table->primaryKey()) = $data[$this->Table->alias()][$this->Table->primaryKey()];
				$dir = call_user_func(array($this, '_getPath'), $entity, $field);
			} else {
				CakeLog::error(sprintf('Cannot get directory to %s.%s: %s pathMethod is not supported.', $this->Table->alias(), $field, $options['pathMethod']));
			}
		}

		$filePathDir = $this->config("uploadFields.$field.path") . (empty($dir) ? '' : $dir . DS);
		$filePath = $filePathDir . $data[$this->Table->alias()][$field];
		$pathInfo = $this->_pathinfo($filePath);

		if (!isset($this->__filesToRemove[$this->Table->alias()])) {
			$this->__filesToRemove[$this->Table->alias()] = array();
		}

		$this->__filesToRemove[$this->Table->alias()][] = $filePath;
		$this->__foldersToRemove[$this->Table->alias()][] = $dir;
		...
		foreach ($options['thumbnailSizes'] as $size => $geometry) {
			$fileName = str_replace(
				...
				array($size, $geometry, $pathInfo['filename'], $entity->get($this->Table->primaryKey()), time(), microtime()),
				$options['thumbnailName']
			);

			$thumbnailPath = $options['thumbnailPath'];
			$thumbnailPath = $this->_pathThumbnail($entity, $field, compact(
				'geometry', 'size', 'thumbnailPath'
			));

			$thumbnailFilePath = "{$thumbnailPath}{$dir}{$directorySeparator}{$fileName}.{$thumbnailType}";
			$this->__filesToRemove[$this->Table->alias()][] = $thumbnailFilePath;
		}
		return $this->__filesToRemove;
	}
